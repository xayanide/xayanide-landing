...
export const POST = async ({ request }) => {
  ...
  try {
    const fileDocuments = await findUploadedFile({ _id: ObjectId.createFromHexString(fileId) }, db, undefined, bucketName);
    ...
    return new Response("The file couldn't be retrieved from the database. Maybe it doesn't exist.", { status: 404 });
  }
};
